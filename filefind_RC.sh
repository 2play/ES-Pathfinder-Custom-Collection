    ...
    case $level in
        ...
        3)
           ...
           if [[ -d $ROMBASE_DIR ]] && [[ $ROMBASE_DIR == $rom_base ]]; then
               ...
               if [[ -z $filefind ]]; then
                   [[ $system_extension == "not found" ]] && continue
...
while read line; do
    ...
    if [[ ${#array[@]} -gt 1 ]]; then
        ...
        dialog_array+=("---BEGIN-CONTENT---")
        dialog_array+=("$line")
        for i in "${array[@]}"; do
            dialog_array+=("$i")
        done
        dialog_array+=("---END-CONTENT---")
        unset array
    fi
...
echo "Das Array enthält ${#dialog_array[@]} Einträge!"
for i in "${dialog_array[@]}"; do
    echo "$i"
done

...
# level 3=super intermediate: /path/system/rom*.{systemext}, 
# level 4=arcade: /path/rom.{systemext}

# experimental level 5=advanced: /path/rom*.{systemext}
# experimental level 6=lastresort: /path/rom*
    ...
    case $level in
        ...
        3)
           ...
           if [[ -d $ROMBASE_DIR ]] && [[ $ROMBASE_DIR == $rom_base ]]; then
               ...
               if [[ -z $filefind ]]; then
                   system_extension=$(system_extension "$rom_system")
                   [[ $system_extension == "not found" ]] && return
